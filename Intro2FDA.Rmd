...
<span style="color:#FF4136">**Example:**</span>  Raw data
...
<span style="color:#FF4136">**Example:**</span>  Pre-processed data
```{r, fig.align='center', out.width = "80%", echo=TRUE}
...
<span style="color:#FF4136">**Example:**</span>  Pre-processed data - 1st derivative
```{r, fig.align='center', out.width = "80%", echo=TRUE}
...
<span style="color:#FF4136">**Example:**</span>  Pre-processed data - 2nd derivative
```{r, fig.align='center', out.width = "80%", echo=TRUE}
...
- <span style="color:#FF4136">**Inner product**</span> of two functions by
$$
...
$$
The sample functions $\hat{\mu}_n(t)=\bar{X}_n(t)$ and $\hat{c}_n(t,s)$ are viewed as estimators of the population parameters $\mu(t)$ and $c(t,s)$.
...
$$
\textstyle Y_i=\sum_{r=1}^p\beta_rX_r+\varepsilon
$$
<!-- The vector of slope parameters $\boldsymbol{\beta}=(\beta_1,\dots,\beta_p)$ minimizes -->
<!-- $$ -->
<!-- \textstyle R(\boldsymbol{\beta})=E(Y-\sum_{r=1}^p\beta_rX_r)^2. -->
<!-- $$ -->
<!-- Equating the partial derivatives $\partial R(\boldsymbol{\beta})/\partial\beta_\ell$ to zero for $\ell=1,\dots,p$ leads to the solution -->
**Normal equations and solution (population):**
$$
\textstyle \mathbf{C}_{XY}=\mathbf{C}_{XX}\boldsymbol{\beta}\quad\Rightarrow\quad  \boldsymbol{\beta}=\mathbf{C}_{XX}^{-1}\mathbf{C}_{XY},
$$
where<br>
$\mathbf{C}_{XX}=[E(X_rX_\ell),\,1\leq r,\ell\leq p]$ and $\mathbf{C}_{XY}=[E(X_rY),\,1\leq r\leq p]$.
...
<!-- **Solution (population):** -->
<!-- $$ -->
<!-- \textstyle \boldsymbol{\beta}=\mathbf{C}_X^{-1}\mathbf{C}_{XY}. -->
<!-- $$ -->
**Normal equations and solution (sample):**
$$
...
$$
where <br>
$\big[\widehat{\mathbf{C}}_{XX}\big]_{r,\ell}=\frac{1}{n}\sum_{i=1}^n X_{ir}X_{i\ell}$ and $\big[\widehat{\mathbf{C}}_{XY}\big]_r=\frac{1}{n}\sum_{i=1}^n X_{ir}Y_{i}$.
...
$$
where: <br> 
$c_{XX}(t,s)=E(X(t)X(s))$<br> 
$c_{XY}(t)=E(X(t)Y)$<br> 
$\hat{c}_{XX}(t,s)=\frac{1}{n}\sum_{i=1}^n(X_i(t)X_i(s))$<br>
...
- Inverting ... is problematic

**Classical solutions:** Ridge regression and principal component regression


<!-- **Functional regression** -->
<!-- $$ -->
<!-- \begin{align} -->
<!-- Y_i&=\int_a^b\beta(t)X_i(t)+\varepsilon_i\\ -->
<!-- Y_i&\approx\sum_{j=1}^J\beta_jX_i(t_j)+\varepsilon_i\\ -->
<!-- \end{align} -->
<!-- $$ -->

<!-- <span style="color:#FF4136">**Problem:**</span> Nearly linearly dependent regressors (*colinearity*), due to very strong correlations between $X_i(t_j)$ and $X_i(t_{j'})$:  -->
<!-- $$\text{cor}(X_t(t_j),X_t(t_{j'}))\to 1\quad\text{as}\quad|t_j-t_{j'}|\to 0$$ -->
